...
from sqlalchemy import Column, String, Integer, Text, TIMESTAMP, ForeignKey, BigInteger, Index, TypeDecorator, Boolean, Float
...
# =============================================================================
# RAG Feedback & Adaptive Retrieval Models
# =============================================================================

class RAGFeedback(Base):
    """User feedback on RAG chat responses.

    Linked to Langfuse traces via trace_id so scores can be correlated
    with retrieval quality metrics in the Langfuse dashboard.
    """
    __tablename__ = "rag_feedback"
    __table_args__ = (
        Index("ix_rag_feedback_trace", "trace_id"),
        Index("ix_rag_feedback_notebook", "notebook_id", "created_at"),
        Index("ix_rag_feedback_user", "user_id", "created_at"),
    )

    feedback_id = Column(UUID(), primary_key=True, default=uuid.uuid4)
    trace_id = Column(String(255), nullable=False, comment="Langfuse trace ID for correlation")
    query_id = Column(UUID(), nullable=False, comment="Internal query UUID")
    user_id = Column(UUID(), ForeignKey("users.user_id", ondelete="SET NULL"), nullable=True)
    notebook_id = Column(UUID(), ForeignKey("notebooks.notebook_id", ondelete="CASCADE"), nullable=False)
    rating = Column(Integer, nullable=True, comment="Star rating 1-5")
    helpful = Column(Boolean, nullable=True, comment="Thumbs up/down")
    ...
    def __repr__(self) -> str:
        return (
            f"<RAGFeedback(feedback_id={self.feedback_id}, "
            f"rating={self.rating}, helpful={self.helpful})>"
        )


class RAGFeedbackNode(Base):
    """Per-node relevance annotations attached to a feedback record."""
    __tablename__ = "rag_feedback_nodes"
    __table_args__ = (
        Index("ix_rag_feedback_nodes", "feedback_id"),
    )

    id = Column(Integer, primary_key=True, autoincrement=True)
    feedback_id = Column(
        UUID(),
        ForeignKey("rag_feedback.feedback_id", ondelete="CASCADE"),
        nullable=False,
    )
    node_id = Column(String(255), nullable=False)
    node_rank = Column(Integer, nullable=True, comment="0-indexed rank in retrieval results")
    was_relevant = Column(Boolean, nullable=True)

    # Relationships
    feedback = relationship("RAGFeedback", back_populates="node_annotations")

    def __repr__(self) -> str:
        return (
            f"<RAGFeedbackNode(id={self.id}, node_id='{self.node_id}', "
            f"rank={self.node_rank}, relevant={self.was_relevant})>"
        )


class RAGFeedbackInsight(Base):
    """Pre-computed analysis insights from FeedbackAnalyzer.

    Rows expire after `expires_at` and are regenerated by the
    FeedbackAnalyzerWorker background thread.
    """
    __tablename__ = "rag_feedback_insights"
    __table_args__ = (
        Index("ix_rag_feedback_insights_notebook", "notebook_id", "created_at"),
    )

    insight_id = Column(UUID(), primary_key=True, default=uuid.uuid4)
    notebook_id = Column(UUID(), ForeignKey("notebooks.notebook_id", ondelete="CASCADE"), nullable=False)
    ...
    recommended_param_value = Column(Float, nullable=True)
    created_at = Column(TIMESTAMP, default=datetime.utcnow, nullable=False)
    expires_at = Column(TIMESTAMP, nullable=True)

    def __repr__(self) -> str:
        return (
            f"<RAGFeedbackInsight(insight_id={self.insight_id}, "
            f"type='{self.analysis_type}', metric='{self.metric_name}', "
            f"value={self.metric_value})>"
        )


class RAGAdaptiveSetting(Base):
    """Adaptive retrieval parameters derived from feedback analysis.

    Rows are looked up by notebook_id where expires_at > NOW().
    The most recent non-expired row wins.
    """
    __tablename__ = "rag_adaptive_settings"
    __table_args__ = (
        Index("ix_rag_adaptive_notebook", "notebook_id", "intent", "created_at"),
    )

    setting_id = Column(UUID(), primary_key=True, default=uuid.uuid4)
    notebook_id = Column(UUID(), ForeignKey("notebooks.notebook_id", ondelete="CASCADE"), nullable=False)
    intent = Column(String(50), nullable=True, comment="NULL = applies to all intents")
    top_k = Column(Integer, nullable=True)
    similarity_threshold = Column(Float, nullable=True)
    bm25_weight = Column(Float, nullable=True)
    vector_weight = Column(Float, nullable=True)
    reranker_top_k = Column(Integer, nullable=True)
    source = Column(
        String(50),
        nullable=True,
    ...
    created_at = Column(TIMESTAMP, default=datetime.utcnow, nullable=False)
    expires_at = Column(TIMESTAMP, nullable=True, comment="NULL = permanent; set to 24h normally")

    def __repr__(self) -> str:
        return (
            f"<RAGAdaptiveSetting(setting_id={self.setting_id}, "
            f"notebook_id={self.notebook_id}, top_k={self.top_k})>"
        )
