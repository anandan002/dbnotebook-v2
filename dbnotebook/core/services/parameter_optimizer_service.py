...
Reads non-expired rag_feedback_insights for a notebook, translates recommended actions
...
class ParameterOptimizerService(BaseService):
    ...
    def apply_recommendations(
        ...
        """Read insights for a notebook and write updated adaptive settings.
        ...
        actions = self._get_actionable_insights(notebook_id)
        if not actions:
            self._logger.debug(f"No actionable insights for notebook {notebook_id}")
            return False
        ...
        self._write_adaptive_settings(notebook_id, intent, params)
        self._logger.info(
            f"Adaptive settings written | notebook={notebook_id} | params={params}"
        )
        return True
    ...
    def get_adaptive_settings(
        ...
        """Return the most recent non-expired adaptive settings for a notebook.
        ...
        from dbnotebook.core.db.models import RAGAdaptiveSetting
        ...
        with self._db_manager.get_session() as session:  # type: ignore[union-attr]
            ...
            for lookup_intent in ([intent, None] if intent else [None]):
                q = (
                    ...
                    .filter(
                        RAGAdaptiveSetting.notebook_id == notebook_id,
    ...
    def _get_actionable_insights(self, notebook_id: str) -> list:
        """Fetch non-expired, actionable insight rows for a notebook."""
    ...
    def _write_adaptive_settings(
        ...
        """Persist adaptive parameter settings, replacing existing rows for this notebook+intent."""
        from dbnotebook.core.db.models import RAGAdaptiveSetting

        expires_at = datetime.utcnow() + timedelta(hours=_SETTING_TTL_HOURS)

        with self._db_manager.get_session() as session:  # type: ignore[union-attr]
            # Replace existing rows for this notebook+intent combination
