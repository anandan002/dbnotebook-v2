...
Reads rag_feedback records for a notebook, groups them by query intent and
...
class FeedbackAnalyzerService(BaseService):
    ...
    def run_full_analysis(
        ...
        Args:
            notebook_id: Optional UUID to scope analysis to one notebook.
                         When None, all notebooks with sufficient feedback are analysed.
            days: Rolling window of feedback records to consider.

        Returns:
            Dict mapping notebook_id → number of insights written.
        """
        ...
        with self._db_manager.get_session() as session:  # type: ignore[union-attr]
            q = session.query(RAGFeedback.notebook_id).filter(
                RAGFeedback.created_at >= cutoff
            )
            if notebook_id:
                q = q.filter(RAGFeedback.notebook_id == notebook_id)

            # Collect distinct notebook_ids that have enough feedback
            from sqlalchemy import func
            count_q = (
                session.query(
                    RAGFeedback.notebook_id,
                    func.count(RAGFeedback.feedback_id).label("cnt"),
                )
                .filter(RAGFeedback.created_at >= cutoff)
            )
            if notebook_id:
                count_q = count_q.filter(RAGFeedback.notebook_id == notebook_id)
            count_q = count_q.group_by(RAGFeedback.notebook_id)
            notebook_counts = {str(row.notebook_id): row.cnt for row in count_q.all()}

        summary: Dict[str, int] = {}
        for nb_id, cnt in notebook_counts.items():
            if cnt < self.MIN_SAMPLE_SIZE:
                self._logger.debug(
                    f"Skipping notebook {nb_id} — only {cnt} feedback records "
                    f"(min={self.MIN_SAMPLE_SIZE})"
                )
                continue
            written = self._analyse_notebook(nb_id, days=days)
            summary[nb_id] = written
            self._logger.info(
                f"Feedback analysis complete | notebook={nb_id} | insights_written={written}"
            )
    ...
    def _analyse_notebook(self, notebook_id: str, days: int) -> int:
        """Analyse feedback for a single notebook and persist insights.

        Returns:
            Number of insight rows written.
        """
        records = self._fetch_feedback(notebook_id, days)
        insights: List[Dict] = []
        ...
        return self._persist_insights(notebook_id, insights)

    def _fetch_feedback(self, notebook_id: str, days: int) -> List:
        """Return RAGFeedback ORM objects for the given notebook and time window."""
        from dbnotebook.core.db.models import RAGFeedback
        cutoff = datetime.utcnow() - timedelta(days=days)
        ...
        with self._db_manager.get_session() as session:  # type: ignore[union-attr]
            records = (
                ...
                .filter(
                    RAGFeedback.notebook_id == notebook_id,
                    RAGFeedback.created_at >= cutoff,
                )
                .all()
            )
    ...
    def _persist_insights(self, notebook_id: str, insights: List[Dict]) -> int:
        """Write insight rows to rag_feedback_insights.

        Existing non-expired insights for the notebook are deleted first so that
        fresh analysis always wins.
        """
        from dbnotebook.core.db.models import RAGFeedbackInsight

        expires_at = datetime.utcnow() + timedelta(hours=self.INSIGHT_TTL_HOURS)

        with self._db_manager.get_session() as session:  # type: ignore[union-attr]
            # Clear old insights for this notebook
            session.query(RAGFeedbackInsight).filter(
                RAGFeedbackInsight.notebook_id == notebook_id
